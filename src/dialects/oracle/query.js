...
class Query extends AbstractQuery {
     ...
     static formatBindParameters(sql, values, dialect, replacementFunc, options) {
        ...
        sql = sql.replace(/\B\$(\$|\w+)/g, (match, key) => {
            ...
            if (list) {
                if (key.match(/^[1-9]\d*$/)) {
                    key = key - 1;
                    if(typeof values[key] !== 'string' || !values[key].includes('TIMESTAMP')){
    ...
    _run(connection, sql, parameters) {
        ...
        if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
            ...
            if (Object.keys(self.outParameters).length > 0) {
                ...
                let sqlToExec = '';
                let opts = {};
                const isMinorTwelveTwo = semver.valid(this.sequelize.options.databaseVersion) &&
    ...
    _dealWithLongAliasesBeforeSelect(sql) {
        ...
        if (tableAliases.length > 0) {
            tableAliases.forEach(tableAlias => {
                const tableRegex = new RegExp(`${tableAlias.alias
                    .replace(/(\.)/g, '->')
                    .replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}|${tableAlias.alias.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}`, 'g');
    ...
    */
                    ...
                    if (key.indexOf('.') > -1) {
                        ...
                        if (attrs[key.toLowerCase()] === undefined) {
                            if(attrs[key] !== undefined){
                                let typeid = this.model.rawAttributes[attrs[key]].type.toLocaleString();
    ...
    handleInsertQuery(results, metaData) {
        if (this.instance && results.length > 0) {
            ...
            if (Object.prototype.hasOwnProperty.call(this.model.rawAttributes, autoIncrementField) &&
